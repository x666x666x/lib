...
void CTachyonRes::LoadTEX(CString strFILE, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal)
{
...
void CTachyonRes::LoadIMGBUF( CString strGroupID)
{
	...
	for( int i=0; i<nCount; i++)
		LoadIMGBUF( LoadString(&file), nIndex, nTotal); // Img\LoginSrc.TIS, Img\WindowSrc.TIS, Img\MainFrmSrc.TIS, Img\MapSrc.TIS, Img\IconSrc.TIS
}

void CTachyonRes::LoadIMGBUF(CString strFILE, int& nIndex, int nTotal)
{
	CFile file(_T(".\\Data\\") + strFILE, CFile::modeRead | CFile::typeBinary);
	...
	while (dwPOS < dwLENGTH)
	{
		...
		// Creates pointer to TTEXLOADBUF and copies pDATA into m_pTDATA of that pointer
		// p_TEX->m_pTDATA contains img buf to access data cast it to pointer to that variable LPTTEXLOADBUF(pTEX->m_pTDATA)
		pTEX->LoadT3DTEX(
			pDATA,
			dwDATA,
			dwSIZE,
		...
		/*
		string path = ".\\TESTXD1\\" + strFILE;
		CString csave = path.c_str(); CString fsave;
		fsave.Format(csave + "\\%d.png", dwPOS);
		...
		pTEX->SaveImageFile(fsave, D3DXIFF_PNG, pTEX->GetTEX());
		*/

		CString pos;
		pos.Format("ImgBufDecomp\\%u.png", dwID);

		D3DXSaveTextureToFile(pos, D3DXIFF_PNG, pTEX->GetTEX(), NULL);
...
void CTachyonRes::LoadIMG( CString strGroupID)
{
	...
	file.Read( &nCount, sizeof(int)); //  5
	file.Read( &nTotal, sizeof(int)); // 4696

	LPMAPRES pTRESDATA = new MAPRES[nCount]; // pointer to MAPRES array
	LPMAPRES pTRES = new MAPRES[nCount];
	RenderBACK( T3DRES_IMG, 0);

	for( int i=0; i<nCount; i++)
		LoadIMG(LoadStringA(&file), &pTRESDATA[i], &pTRES[i], nIndex, nTotal);
...
void CTachyonRes::LoadIMG(CString strFILE, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal)
{
	/**
	* Img\LoginImg.TIM
	* Img\WindowImg.TIM
	...
	*/
	...
	while (dwPOS < dwLENGTH)
	{
		...
		/*
		CString pos;
		pos.Format("ImgDecomp\\%u.png", dwPOS);

		//D3DXSaveTextureToFile(pos, D3DXIFF_PNG, NULL);
		*/
...
void CTachyonRes::LoadANI(DWORD dwFileID, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal)
{
...
void CTachyonRes::LoadMESH(DWORD dwBASE, DWORD dwFileID, LPMAPRES pTRES, int& nIndex, int nTotal)
{
...
void CTachyonRes::LoadOBJ(CString strFILE, LPMAPRES pTRES, int& nIndex, int nTotal)
{
	CFile file(_T(".\\Data\\") + strFILE, CFile::modeRead | CFile::typeBinary);
...
void CTachyonRes::LoadSFX(CString strFILE, LPMAPRES pTRES, int& nIndex, int nTotal)
{
	CFile file(_T(".\\Data\\") + strFILE, CFile::modeRead | CFile::typeBinary);
	...
	for( it = m_mapIMG.begin(); it != m_mapIMG.end(); it++)
	{
		...
		if(pIMG)
		{
			...
			for( int i=0; i<nCount; i++)
			{
				...
				/*
				CString pos;
				pos.Format("ImgTestDecomp\\%d.png", imgId);
				pIMG->m_vImage[i]->SaveImageFile(CTachyonRes::m_pDEVICE->m_pDevice, pos, D3DXIFF_PNG); // save image
				imgId++;
				*/
			}
		}
