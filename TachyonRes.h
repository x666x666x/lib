...
public:
	...
	void LoadIMGBUF(CString strFILE, int& nIndex, int nTotal);

	void LoadIMG(CString strGroupID);
	void LoadIMG(CString strFILE, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal);

	void LoadMEDIA( CString strGroupID);
	void LoadMEDIA(DWORD dwID, CString strFILE, DWORD dwPOS);

	void LoadMESH( CString strGroupID);
	void LoadMESH(DWORD dwBASE, DWORD dwFileID, LPMAPRES pTRES, int& nIndex, int nTotal);

	void LoadTEX( CString strGroupID);
	void LoadTEX(CString strFILE, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal);

	void LoadANI( CString strGroupID);
	void LoadANI(DWORD dwFileID, LPMAPRES pTRESDATA, LPMAPRES pTRES, int& nIndex, int nTotal);

	void LoadOBJ( CString strGroupID);
	void LoadOBJ(CString strFILE, LPMAPRES pTRES, int& nIndex, int nTotal);

	void LoadSFX( CString strGroupID);
	void LoadSFX(CString strFILE, LPMAPRES pTRES, int& nIndex, int nTotal);
